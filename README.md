...
- `Encode`
  - 空间分支：`SegFormer` 提取区域级空间图像的语义分割结果与空间向量。当前数据里如同时存在街景图和卫星图，会统一作为 `image_paths` 的普通图像输入，而不是走独立遥感分支
...
- `image_paths`
  同一区域可放多张空间图像，当前版本支持把街景图、实拍图、卫星图一起放进来，训练时统一按区域聚合；但当前不会对卫星图做独立建模。
- `current_texts`
...
2. 每个 `region_id` 收集多张空间图像
   当前可直接使用街景图；如果像现在这版数据一样已经有卫星图，也可以一并放入 `image_paths`
